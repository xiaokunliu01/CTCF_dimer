setwd("/home/lxk/private/DNase-C/dimer_paper/fig1/K562/CTCF_HiChIP/R2/figs2e/")

library(tidyverse)

df <- read.table("figs2e.tab", sep = "\t", header = FALSE)
df[,1] <- factor(df[,1], levels = c("hh", "ht", "th", "tt"))

# png("cis_trans_frequency.png", units = "in", width = 5, height = 4, res = 300, bg = "transparent")
pdf("figs2e.pdf", width = 2.8, height = 2.6, bg = "transparent")
