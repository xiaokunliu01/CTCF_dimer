setwd(dir = "/home/lxk/private/DNase-C/dimer_paper/fig1/293T/DNaseC/R2/fig1h/")
library(dplyr)
library(tidyverse)
library(ggpubr)
...
df <- read.table("/mnt/disk5/1/DNaseC/total/293T/dimer/CTCF/interaction_footprint.tab", sep = "\t", header = FALSE)
...
df = df %>% filter(df[,6]<=50 & df[,7]<=50 & abs(df[,4])<=3 & abs(df[,5])<=3 & df[,8]>=100 & df[,8]<1000)
...
pdf("interaction_footprint_density_5p_dis3p_-3_0.1_1kb_merge.pdf", width = 2.5, height = 2.5, bg = "transparent")

ggplot(df, aes(x=df[,3], y=df[,2])) +
  # stat_density_2d(aes(fill = ..density..), geom = "raster", contour = FALSE) +
  stat_density_2d(aes(fill = after_stat(level)), geom = "polygon", contour = TRUE, adjust = 1.5, bins=10) +
...
annotate("text", x=-15, y=10, label="91.7%", colour = "red",size = 12*0.3528) + 
  annotate("text", x=15, y=10, label="8.3%",size = 12*0.3528) + 
