setwd(dir = "/home/lxk/private/DNase-C/dimer_paper/fig1/K562/HiChIP/R3/")
library(dplyr)
library(tidyverse)
library(ggpubr)
...
df <- read.table("/mnt/disk1/6/lxk/private/HiChIP/total/K562/dimer/interaction_footprint.tab", sep = "\t", header = F)
...
ggplot(df, aes(x=df[,3], y=df[,2])) +
  ...
  annotate("text", x=-35, y=45, label="36.9%", colour = "red",size = 12*0.3528) + 
  annotate("text", x=35, y=45, label="24.3%",size = 12*0.3528) + 
  annotate("text", x=-35, y=-45, label="24.1%",size = 12*0.3528) + 
  annotate("text", x=35, y=-45, label="14.7%",size = 12*0.3528) + 
