...
# library(openxlsx)
library(ggplot2)
setwd('/mnt/disk1/6/lxk/private/DNase-C/dimer_paper/fig5/spDamID/PCA/')
data <- read.table("GATC.bed")
...
ggplot(df1) +
  ...
  labs(title = "CTCF spDamID", x=xlab1, y=ylab1) + 
  coord_cartesian(
    xlim = c(-110, 110),
    ylim = c(-110, 110),
  ) +
  ...
  scale_colour_manual(
    ...
    breaks = c("CC", "CCswap"),
    labels = c("FL", expression(Delta *"(233-265)"))
    ) +
  theme(
...
ggsave("pca_GATC_R1.pdf", width = 3.1, height = 2.0, dpi = 300)
