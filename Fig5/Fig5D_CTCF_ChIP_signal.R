setwd("/mnt/disk1/6/lxk/private/DNase-C/dimer_paper/fig5/ChIP/CTCF/")
library(tidyverse)
...
df <- read.table("ChIP_CTCF_C2D5_all_CTCFmotif_profile.tab", sep = "\t", header = FALSE, skip = 2)
df <- as.data.frame(t(data.frame(df[,-2], row.names = 1)))
bins <- seq(from = -1000, to = 995, by = 5)
df <- cbind(bins, df)

# png("CTCF_spike_in.png", units = "in", width = 2.8, height = 2.8, res = 300, bg = "white")
pdf("CTCF_spike_in.pdf", width = 3.2, height = 2.5, bg = "transparent")

ggplot(data = df, mapping = aes(x = bins)) + 
  geom_line(aes(y = df[,2]/1, color = "blk"), linewidth = 1.2) +
  geom_line(aes(y = df[,3]/0.043, color = "Y226A; F228A"), linewidth = 1.2) +
  geom_line(aes(y = df[,4]/0.041, color = "del 232-265"), linewidth = 1.2) +
  geom_line(aes(y = df[,5]/0.058, color = "hCTCF"), linewidth = 1.2) +
  scale_color_manual(
