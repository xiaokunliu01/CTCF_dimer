setwd("/mnt/disk1/6/lxk/private/DNase-C/dimer_paper/fig3/RAD21_degron/RAD21_ChIP/R2/")

library(tidyverse)
png("R3D7_RAD21_spike_in.png", units = "in", width = 2.8, height = 2.8, res = 300, bg = "white")
...
df <- read.table("ChIP_RAD21_R3D7_all_CTCFmotif_profile.tab", sep = "\t", header = FALSE, skip = 2)
df <- as.data.frame(t(data.frame(df[,-2], row.names = 1)))
bins <- seq(from = -1000, to = 995, by = 5)
df <- cbind(bins, df)

# png("R3D7_RAD21_spike_in.png", units = "in", width = 2.8, height = 2.8, res = 300, bg = "white")
pdf("R3D7_RAD21_spike_in.pdf", width = 3.2, height = 2.5, bg = "transparent")

ggplot(data = df, mapping = aes(x = bins)) + 
  geom_line(aes(y = df[,2]/1, color = "DMSO"), size = 1.2) +
  geom_line(aes(y = df[,3]/1.76, color = "dTAG"), size = 1.2) +
