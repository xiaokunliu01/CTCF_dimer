setwd("/home/lxk/private/DNase-C/dimer_paper/fig3/RAD21_degron/trans/dimer/")

library(tidyverse)

df <- read.table("fig3i.tab", sep = "\t", header = FALSE)
df[,1] <- factor(df[,1], levels = c("hh", "ht", "th", "tt"))

# png("cis_trans_frequency.png", units = "in", width = 5, height = 4, res = 300, bg = "transparent")
pdf("fig3i.pdf", width = 2.3, height = 2.6, bg = "transparent")
...
ggplot(data = df, mapping = aes(x = df[,4], y = df[,3], fill= df[,1])) + 
  ...
  xlab(" ") + 
  ylab("Proportion (%)") + 
  labs(title=expression(italic("trans"))) + 
