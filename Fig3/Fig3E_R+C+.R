setwd("/home/lxk/private/DNase-C/dimer_paper/fig3/reHiChIP/dimer/R2/")

library(tidyverse)
library(ggtext)

df <- read.table("R+C+.tab", sep = "\t", header = F)
df[,1] <- factor(df[,1], levels = c("hh", "ht", "th", "tt"))

# png("cis_trans_frequency.png", units = "in", width = 5, height = 4, res = 300, bg = "transparent")
pdf("R+C+.pdf", width = 2.8, height = 2.6, bg = "transparent")
...
ggplot(data = df, mapping = aes(x = df[,4], y = df[,3], fill= df[,1])) + 
  ...
  xlab("d1 & d2 (bp)") + 
  ylab("Proportion (%)") + 
  labs(title = "RAD21<span style='color:red;'>+</span>CTCF+") +
