...
df = df %>% filter(df[,1]=="TandemF")

# png("interaction_footprint_5p.png", units = "in", width = 5, height = 5, res = 300, bg = "transparent")
pdf("interaction_footprint_5p_dis3p_-3_TanF.pdf", width = 2.5, height = 2.5, bg = "transparent")
...
ggplot(df, aes(x=-(df[,3]), y=df[,2])) +
  ...
  annotate("text", x=-15, y=10, label="94.0%", colour = "red",size = 12*0.3528) + 
  annotate("text", x=15, y=10, label="3.4%",size = 12*0.3528) + 
  annotate("text", x=-15, y=-10, label="2.6%",size = 12*0.3528) + 
