setwd("/home/lxk/private/DNase-C/dimer_paper/fig1/293T/DNaseC/R2/fig1i/")

library(tidyverse)

df <- read.table("fig1i.tab", sep = "\t", header = F)
df[,1] <- factor(df[,1], levels = c("hh", "ht", "th", "tt"))

# png("cis_trans_frequency.png", units = "in", width = 5, height = 4, res = 300, bg = "transparent")
pdf("fig1i.pdf", width = 2.3, height = 2.6, bg = "transparent")
...
ggplot(data = df, mapping = aes(x = df[,4], y = df[,3], fill= df[,1])) + 
  ...
  scale_x_discrete(
    limits = c("cis","trans"),
    labels = c(expression(italic("cis")), expression(italic("trans"))),
    expand = c(0,0)
  ) +
  scale_y_continuous(
