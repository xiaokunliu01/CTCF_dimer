...
df = df %>% filter(df[,1]=="Con")

# png("interaction_footprint_5p.png", units = "in", width = 5, height = 5, res = 300, bg = "transparent")
pdf("interaction_footprint_5p_dis3p_-3_con.pdf", width = 2.5, height = 2.5, bg = "transparent")
...
ggplot(df, aes(x=df[,3], y=df[,2])) +
  ...
  annotate("text", x=-15, y=10, label="91.5%", colour = "red",size = 12*0.3528) + 
  annotate("text", x=15, y=10, label="4.2%",size = 12*0.3528) + 
  annotate("text", x=-15, y=-10, label="4.1%",size = 12*0.3528) + 
