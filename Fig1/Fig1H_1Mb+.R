...
df = df %>% filter(df[,6]<=50 & df[,7]<=50 & abs(df[,4])<=3 & abs(df[,5])<=3 & df[,8]>=1000000)
...
pdf("interaction_footprint_density_5p_dis3p_-3_1Mb+_merge.pdf", width = 2.5, height = 2.5, bg = "transparent")
...
ggplot(df, aes(x=df[,3], y=df[,2])) +
  ...
  annotate("text", x=-15, y=10, label="87.5%", colour = "red",size = 12*0.3528) + 
  annotate("text", x=15, y=10, label="12.5%",size = 12*0.3528) + 
