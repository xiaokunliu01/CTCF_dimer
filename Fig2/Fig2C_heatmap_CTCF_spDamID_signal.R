setwd(dir = "/home/lxk/private/DNase-C/dimer_paper/fig2/2C/R4/")

library(ggplot2)
library(reshape2)

df <- read.table("DCAC_DCSAC.bdg", sep = "\t", header = FALSE)
colnames(df) <- c("chr", "start", "end", "NT", "Swap")
...
pdf("fig2C_heatmap_40bp.pdf", width = 1.0, height = 3.3, bg = "transparent")
...
ggplot(long_df, aes(x = Sample, y = row_id, fill = Expression)) +
  ...
  scale_x_discrete(
    ...
    # labels = c(NULL, NULL, NULL, NULL, NULL),
    expand = c(0,0)
  ) +
  scale_y_continuous(
    expand = c(0,0)
