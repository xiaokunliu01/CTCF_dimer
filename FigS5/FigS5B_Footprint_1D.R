setwd("/mnt/disk1/6/lxk/private/DNase-C/dimer_paper/figS5/RC1A3_Footprint_1D/")
...
df <- read.table("DNaseC_RC1A3_all_CTCFmotif_profile.tab", sep = "\t", header = FALSE, skip = 2)
...
pdf("RC1A3_FootprintC_1D_500bp.pdf", width = 3.2, height = 2.5, bg = "transparent")

ggplot(data = df, mapping = aes(x = bins)) + 
  geom_line(aes(y = df[,2]/1, color = "DMSO"), size = 1.2) +
  geom_line(aes(y = df[,3]/1, color = "dTAG"), size = 1.2) +
