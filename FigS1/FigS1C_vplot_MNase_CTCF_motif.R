setwd(dir = "/mnt/disk1/6/lxk/private/DNase-C/dimer_paper/figS1/vplot/")
library(tidyverse)
library(colorspace)
# png("FragmentLengthVsDistance_K562_FA_DHS_q220.png", units = "in", width = 5, height = 3, res = 300, bg = "transparent")
pdf("FragmentLengthVsDistance_CTCF.pdf", width = 3.2, height = 3, bg = "transparent")

df <- read.table("FragmentLengthVsDistance_CTCF.csv", sep = ";", header = F)
...
ggplot(df, aes(x=df[,2], y=df[,1])) + 
  ...
  # geom_vline(xintercept = 53.33, color="red", size = 1.2) +
  annotate("text", x=-60, y=10, label="MNase", colour = "red", size=4.5) +
  # CTCF motif 64.4%
  # DHS 65.2%
