...
pdf("enrichment_dDHS.pdf", width = 3.2, height = 3, bg = "transparent")

df <- read.table("DNaseC_dDHS_profile.tab", sep = "", header = F, skip = 2)
df <- as.data.frame(t(data.frame(df, row.names = 1)))
dis <- c(-500:500)
df <- cbind(dis, df)

ggplot(data = df, mapping = aes(x = dis)) + 
  geom_line(aes(y = df[,2], color = "DNase"), size = 1.2) +
  geom_line(aes(y = df[,3], color = "MNase"), size = 1.2) +
  scale_color_manual(
    limits = c(
      "MNase", 
      "DNase"
