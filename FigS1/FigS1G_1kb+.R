...
ggplot(data = df, mapping = aes(x = df[,4], y = df[,3], fill= df[,1])) + 
  ...
  scale_x_discrete(
    limits = c("FootprintC","MicroC","insituHi-C"),
    labels = c("Footprint-C", "Micro-C", expression(italic("in situ") * " Hi-C")),
    expand = c(0,0)
  ) +
  scale_y_continuous(
    # breaks=seq(0,150,50),
